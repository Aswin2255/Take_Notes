...
import { getUsersession, logOut } from "../api/api";
import { Authaction } from "../Redux/Userslices";
import moments from "moment";
import Loader from "./Loader";

function Sessionmodal({ modal }) {
    const sessionid = useSelector((state)=>state.Auth.sessionid)
  const [loader, setloader] = useState(true);
  const [userdata, setuserdata] = useState([]);
  const dispatch = useDispatch();
  useEffect(() => {
    getUsersession("/auth/getusersession")
      .then(({ data }) => {
  ...
  const columns = [
    {
      name: "starttime",
      selector: (row) =>
        moments(row.starttime).format("MMMM Do YYYY, h:mm:ss a"),
      sortable: true,
    },
    {
      name: "endtime",
      selector: (row) =>
        row.endtime
          ? moments(row.endtime).format("MMMM Do YYYY, h:mm:ss a")
          : "-",
      sortable: true,
    },
